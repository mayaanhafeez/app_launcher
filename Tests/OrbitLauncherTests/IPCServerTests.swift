...
/// Mirrors `orbitctl`'s client exactly (connect, write, read once), plus a receive
...
private func startIPCServer(_ recorder: CommandRecorder) throws -> (server: IPCServer, path: String, directory: URL) {
    let directory = orbitTemporaryDirectory("orbit-ipc")
...
@Test func ipcRebindsOverAStaleSocketFile() async throws {
    ...
    let url = directory.appendingPathComponent("orbit.sock")
