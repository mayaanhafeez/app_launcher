...
@Test func watcherFiresOnAnInPlaceRewrite() async throws {
    let directory = orbitTemporaryDirectory("orbit-watch")
    ...
    try orbitRewriteInPlace(config, "return { items = { { id = 'root', label = 'Go' } } }")

    #expect(await orbitWaitUntil(timeout: 3) { changes.value > 0 })
}
...
@Test func watcherRearmsAfterASaveByRename() async throws {
    ...
    try orbitRewriteInPlace(config, "return { items = { { id = 'root', label = 'Go' } } }")
    #expect(await orbitWaitUntil(timeout: 3) { changes.value > seen })
}
...
@Test func fileOnlyWatcherSeesWritesWithoutADirectoryWatch() async throws {
    ...
    try orbitRewriteInPlace(pointer, "everforest")
    #expect(await orbitWaitUntil(timeout: 3) { changes.value > 0 })
}

@Test func watcherStartsEvenWhenTheDirectoryIsAbsent() async throws {
    // A fresh install has no ~/.config/orbit at all; the watcher creates it rather
    // than throwing, so the first save after `Open Config Folder` is still seen.
    let root = orbitTemporaryDirectory("orbit-watch")
    defer { orbitRemove(root) }
    let directory = root.appendingPathComponent("nested/orbit")
