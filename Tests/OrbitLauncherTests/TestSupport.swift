...
func orbitTemporaryDirectory(_ prefix: String = "orbit-test") -> URL {
    let url = URL(fileURLWithPath: "/tmp").appendingPathComponent("\(prefix)-\(UUID().uuidString.prefix(8))")
    try? FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
    return url
}

func orbitRemove(_ url: URL) { try? FileManager.default.removeItem(at: url) }
...
@discardableResult
func orbitWaitUntil(timeout: TimeInterval = 5, _ condition: @Sendable () -> Bool) async -> Bool {
...
func orbitRewriteInPlace(_ url: URL, _ contents: String) throws {
...
func orbitLoadConfig(
    _ source: String?,
    files: [String: String] = [:],
    timeout: TimeInterval = 5
) async -> (runtime: LuaRuntime, load: ConfigLoad, directory: URL) {
    let directory = orbitTemporaryDirectory("orbit-lua")
    ...
    _ = await orbitWaitUntil(timeout: timeout) { finished.value }
...
func orbitProviderOutcome(
    ...
    _ = await orbitWaitUntil(timeout: wait) { box.value != nil }
    return box.value
}
