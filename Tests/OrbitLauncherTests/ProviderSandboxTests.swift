...
@Test func providerRowsDecodeWithTheirActions() async {
    let (runtime, load, directory) = await orbitLoadConfig(providerConfig)
    defer { orbitRemove(directory) }
    #expect(load.error == nil)

    let outcome = await orbitProviderOutcome(runtime, name: "rows", query: "hello", timeout: 5)
...
@Test func providerStateHasNoExecutionGlobals() async {
    ...
    let outcome = await orbitProviderOutcome(runtime, name: "sandbox", timeout: 5)
    guard let outcome else { Issue.record("provider never completed"); return }
    #expect(outcome.error == nil)
    // io, os.execute, terminal, run, osascript, orbit.can_execute — the config state
...
@Test func providerExceedingItsDeadlineFailsCleanly() async {
    ...
    let completed = await orbitProviderOutcome(runtime, name: "countdown", timeout: 5)
    #expect(completed?.error == nil)
    #expect(completed?.labels == ["done 5000050000"])

    let started = Date()
    let expired = await orbitProviderOutcome(runtime, name: "countdown", timeout: 0)
...
@Test func infiniteProviderIsStoppedByTheBudget() async {
    ...
    let started = Date()
    let outcome = await orbitProviderOutcome(runtime, name: "spin")
    ...
    let after = await orbitProviderOutcome(runtime, name: "sandbox", timeout: 5)
...
@Test func providerErrorsAreSurfacedNotSwallowed() async {
    ...
    let outcome = await orbitProviderOutcome(runtime, name: "boom", timeout: 5)
...
@Test func nonSerializableProviderIsRejected() async {
    ...
    let outcome = await orbitProviderOutcome(runtime, name: "native", timeout: 5)
...
@Test func unknownProviderYieldsNoRows() async {
    ...
    let outcome = await orbitProviderOutcome(runtime, name: "not-declared", timeout: 5)
    #expect(outcome?.error == nil)
    #expect(outcome?.rows.isEmpty == true)
}
