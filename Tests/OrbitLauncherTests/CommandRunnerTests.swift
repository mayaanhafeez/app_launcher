...
@Test func queryIsSubstitutedAndShellQuoted() async {
    let directory = orbitTemporaryDirectory("orbit-cmd-injection")
...
@Test func rowsAreCachedByResolvedCommand() async {
    let directory = orbitTemporaryDirectory("orbit-cmd-cache")
    defer { orbitRemove(directory) }
...
@Test func cancelDropsAPendingRun() async {
    ...
    _ = await orbitWaitUntil(timeout: 2) { box.value != nil }
...
@Test func commandRowsFollowTheStaticRows() async {
    ...
    _ = await orbitWaitUntil(timeout: 10) { labels.value.contains("FromCommand") }
...
@Test func navigatingAwayCancelsTheCommand() async {
    ...
    controller.open(route: "search")
    _ = controller.back()
    _ = await orbitWaitUntil(timeout: 2) { labels.value.contains("FromCommand") }
