...
// on (or disturbs) a real `~/.config/kitsune`.

@Test func missingConfigFallsBackToDefaultNodes() async {
    let (runtime, load, directory) = await kitsuneLoadConfig(nil)
...
@Test func syntaxErrorIsReportedNotSwallowed() async {
    let (runtime, load, directory) = await kitsuneLoadConfig("return { items = {")
    defer { kitsuneRemove(directory); _ = runtime }
...
@Test func configReturningANonTableIsReported() async {
    let (runtime, load, directory) = await kitsuneLoadConfig("return 42")
    defer { kitsuneRemove(directory); _ = runtime }
...
@Test func configStateHasExecutionGlobalsButNoIO() async {
    ...
    return {
      items = {
        ...
        { id = "kitsune", label = tostring(kitsune.can_execute) .. "|" .. kitsune.config_dir .. "|" .. package.cpath },
      },
    }
    """)
    defer { kitsuneRemove(directory); _ = runtime }
    ...
    #expect(load.node("kitsune")?.label == "true|\(directory.path)|")
}
