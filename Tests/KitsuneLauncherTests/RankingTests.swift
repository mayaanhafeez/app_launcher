...
@Test func backRowIsNotAUse() {
    ...
    #expect(controller.usage.bonus(for: "kitsune.back") == 0)
}
...
@Test func usageRoundTripsThroughItsFile() async {
    let directory = kitsuneTemporaryDirectory("kitsune-usage")
    ...
    _ = await kitsuneWaitUntil(timeout: 10) { FileManager.default.fileExists(atPath: url.path) }
...
@Test func aMemoryOnlyStoreWritesNothing() async {
    ...
    let directory = kitsuneTemporaryDirectory("kitsune-usage-none")
    defer { kitsuneRemove(directory) }
    let store = UsageStore(url: nil)
    store.record("a")
    #expect(store.bonus(for: "a") > 0)
