...
if arguments.first == "--help" || arguments.first == "help" {
    print("Usage: kitsunectl [toggle|show|hide|invoke|list|reload|theme|version|ping] [route-or-node] [query...]")
    print("       kitsunectl list apps            # rows the apps menu would show, as JSON")
    print("       kitsunectl list root chrome     # rows for a query at the top level")
    exit(0)
}
...
guard bytes.count <= MemoryLayout.size(ofValue: address.sun_path) else { fputs("kitsunectl: socket path too long\n", stderr); exit(1) }
...
guard connected == 0 else { fputs("kitsunectl: KitsuneLauncher is not running\n", stderr); exit(1) }
...
guard !payload.isEmpty, let response = try? JSONDecoder().decode(Response.self, from: payload) else { fputs("kitsunectl: invalid response\n", stderr); exit(1) }
