...
struct MenuNode: Sendable {
    ...
    var iconPath: String = ""
    /// Extra route names for `kitsunectl show <alias>`; also searchable.
    var aliases: [String] = []
    let provider: String?
    ...
    var keep: Bool = false
    /// Never listed, but still reachable by search, alias and `kitsunectl invoke` — the
...
enum HotKeyTarget: Sendable, Equatable {
    ...
    /// `MenuController.invoke(id:)`, the same entry point `kitsunectl invoke` uses.
    case invoke(String)
}
